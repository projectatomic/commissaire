...
class ClusterJoinResource(Resource):
    """
    Resource to allow hosts to join a cluster directly.
    """

    def on_put(self, req, resp, name):
        """
        Handles PUT requests from hosts to join a cluster.
        ...
        :param resp: Response instance that will be passed through.
        :type resp: falcon.Response
        :param name: The name of the Cluster to join.
        :type name: str
        """

        if not util.etcd_cluster_exists(self.store, name):
            self.logger.info(
                'Join request for nonexistent cluster {0}'.format(name))
            resp.status = falcon.HTTP_404
            return
        ...
        try:
            ...
            self.logger.info(
                'Unable to determine host address in join request '
                'for cluster {0}'.format(name))
            resp.status = falcon.HTTP_400
            return
        ...
        try:
            ...
            self.logger.info(
                'Bad join request for cluster {0}'.format(name))
            resp.status = falcon.HTTP_400
            return

        resp.status, host_model = util.etcd_host_create(
            self.store, address, ssh_priv_key, name)
