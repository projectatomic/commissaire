...
class Test_ClusterJoinResource(TestCase):
    """
    Tests for the ClusterJoin resource.
    """
    ...
    def before(self):
        ...
        self.datasource.write = MagicMock(name='set')
        self.datasource.write.return_value = self.return_value
        self.resource = clusters.ClusterJoinResource(self.datasource)
        self.api.add_route('/api/v0/cluster/{name}/join', self.resource)

    def test_cluster_join(self):
        """
        Verify directly joining a cluster.
        """
        ...
        body = self.simulate_request(
            '/api/v0/cluster/testing/join', method='PUT', body=data)
        self.assertEquals(1, self.datasource.set.call_count)
        self.assertEquals(1, self.datasource.write.call_count)
        ...
        body = self.simulate_request(
            '/api/v0/cluster/testing/join', method='PUT', body=bad_data)
        # datasource's set should not have been called
        self.assertEquals(0, self.datasource.set.call_count)
